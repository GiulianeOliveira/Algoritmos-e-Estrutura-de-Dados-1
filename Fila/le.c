 ...
 */
 elem * create_node(int val){ 
    struct elem *cab;
    cab = malloc(sizeof(struct elem));  //CRIA CABEÇA
    if(cab == NULL){
        return NULL;
    }
    else
...
int delete_l(struct llist *desc, elem * prev){
    //Consideando prev->next como elemento a ser apagado
    
    if(prev == NULL){ // OU prev
        if(desc->cabeca == NULL){
            return 0;
        }
        else{
        ...
        if(prev->next == NULL){ //Nao o que apagar
            return 0;
        }
        else{
            struct elem *tp;
